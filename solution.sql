...
INSERT
	...
	brand_origin_id)
SELECT
	DISTINCT TRIM(split_part(s.auto, ',', 1)) AS brand_name,
	bo.brand_origin_id
FROM
	raw_data.sales s
...
JOIN car_shop.brand b ON
	b.brand_name = TRIM(split_part(s.auto, ',', 1))
JOIN car_shop.color c ON
	c.color_name = TRIM(split_part(s.auto, ',', 2))
JOIN car_shop.purchaser p ON
	p.name = s.person;
...
SELECT 
	round(
		COUNT(*) FILTER (WHERE s.gazoline_consumption IS NULL)::NUMERIC / COUNT(*) * 100, 2   
...
SELECT 
	p.name AS person,
	string_agg(b.brand_name, ', ')
FROM
	car_shop.brand b
JOIN car_shop.sales s ON
	b.brand_id = s.brand_id
JOIN car_shop.purchaser p ON
	s.purchaser_id = p.purchaser_id
GROUP BY
	p.name,
	s.purchaser_id;
